<?php 
    ...
    $status = $_POST['status'];
    // $status = 'Pending';
    $_SESSION['fifo_hpercode'] = $_POST['hpercode'];
    ...
    foreach ($data as $row) {
        if($row['hpercode'] === $_SESSION['fifo_hpercode']){
            $type_color;
            if($row['type'] == 'OPD'){
                $type_color = '#d77707';
            }else if($row['type'] == 'OB'){
    ...
    foreach ($data as $row) {
        ...
        if($row['hpercode'] == $_SESSION['fifo_hpercode'] && $row['status'] == 'Pending'){
            $style_tr = 'opacity:1; pointer-events:auto;';
        }else if($row['status'] == 'Pending' && $row['hpercode'] != $_SESSION['fifo_hpercode']){
